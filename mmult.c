...
int main(int argc, char **argv)
{
    ...
    nbytes = 0;                     /* just to make the complier happy */
    ...
    if (taskid == MASTER) {
        ...
        for (dest=1; dest<=numworkers; dest++) {
            ...
            count = rows*NCA;
            MPI_Send(a[offset], count, MPI_DOUBLE, dest, mtype, MPI_COMM_WORLD);
            count = NCA*NCB;
    ...
    if (taskid > MASTER) {
        ...
        printf("task %d: waiting for matrix from master...\n", taskid);
        ...
        printf("task %d: got matrix from master...\n", taskid);
